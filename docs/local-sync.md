...
3. On the other app, choose receive and enter one of the displayed private IP
   addresses, the port and the 32-character pairing code.
...
It uses a temporary random TCP port and accepts private IPv4 connections only. The
pairing code is random, single-use session material; authenticated AES-GCM frames
detect an incorrect code or changed traffic. Use a network you trust and do not
forward the port through a router.
...
catalogs if validation fails. The transfer exposes no API for arbitrary filesystem
paths or remote writes; only a validated progress snapshot can be previewed and
applied.
