...
public class PracticeCardMultipelAnswersBase : ComponentBase
{
    // Content
    ...
    [Parameter] public List<string> MissingAnswers { get; set; } = [];
    [Parameter] public List<string> AnswersGiven { get; set; } = [];
    protected string MissingAnswersDisplay => MissingAnswers.Aggregate("", (agg, ans) => agg + ans + ", ").TrimEnd(' ', ',');
    protected string AnswersGivenDisplay => AnswersGiven.Aggregate("", (agg, ans) => agg + ans + ", ").TrimEnd(' ', ',');
